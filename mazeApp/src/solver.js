const importEntries = require('./entries');
...
Solver.prototype.solve = function () {
    const startPosition = importEntries.getEntryNode(this.maze.entryNodes, 'start');
    const endPosition = importEntries.getEntryNode(this.maze.entryNodes, 'end');
...
Solver.prototype.generateSolveNodes = function (start, end) {
    ...
    for (let y = 0; y < rowCount; y++) {
        ...
        for (let x = 0; x < rowLength; x++) {
            if (utilsImport.stringVal(matrix[y], x)) {
                // Walls don't have connections.
                continue;
            }

            const nswe = {
                'n': (0 < y) && utilsImport.stringVal(matrix[y - 1], x),
                's': (rowCount > y) && utilsImport.stringVal(matrix[y + 1], x),
                'w': (0 < x) && utilsImport.stringVal(matrix[y], (x - 1)),
                'e': (rowLength > x) && utilsImport.stringVal(matrix[y], (x + 1))
            }
...
Solver.prototype.drawAstarSolve = function () {
    ...
    if (hasGates) {
        startNode = this.start;
        endNode = this.finish;
        const gateEntry = importEntries.getEntryNode(this.maze.entryNodes, 'start', true);
    ...
    if (hasGates) {
        const gateExit = importEntries.getEntryNode(this.maze.entryNodes, 'end', true);
        //ctx.fillRect((gateExit.x * wallSize), (gateExit.y * wallSize), wallSize, wallSize);
    }
};

Solver.prototype.draw = function (maze) {
    //todo
    var alasArray = [];
    ...
    while (finished === false) {
        ...
        if (-1 !== ['w', 'e'].indexOf(node.last_step)) {
            ...
            //TODO
            let temp = {
                from_x: (start * wallSize),
                from_y: (node.y * wallSize),
                width: to_x,
                height: wallSize,
            };
            alasArray.push(temp)
            ...
            //console.log((start * wallSize), (node.y * wallSize), to_x, wallSize);
        ...
        if (-1 !== ['n', 's'].indexOf(node.last_step)) {
            ...
            //TODO
            let temp = {
                from_x: (node.x * wallSize),
                from_y: (start * wallSize),
                width: wallSize,
                height: to_y
            };
            ...
            alasArray.push(temp)
            //console.log((node.x * wallSize), (start * wallSize), wallSize, to_y);
            //ctx.fillRect((node.x * wallSize), (start * wallSize), wallSize, to_y);
        }

        node = nodes[i];

        if (prevZuz !== 2 && ((prevZuz === 1 && zuz === 1) || (prevZuz !== 1 && zuz !== 1))) {
            counter++;
        }
    ...
    maze.pathLength = importData.pathLenght(alasArray);
    maze.pathCross = importData.pathCross(alasArray, maze.matrix)

    //console.log(alasArray);
    //console.log(counter);
};
