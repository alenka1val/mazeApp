...
function Maze(args) {
    ...
    this.cross = parseInt(settings['cross'], 10);
    this.pathLength = parseInt(settings['pathLength'], 10);
    this.pathLength = parseInt(settings['pathCross'], 10);
}
...
Maze.prototype.parseMaze = function (nodes) {
    ...
    nodes[position] = utilsImport.replaceAt(nodes[position], 0, 1);
    ...
    while (visited < (mazeSize - 1)) {
        ...
        let directions = Object.keys(next).filter(function (key) {
            return (-1 !== next[key]) && !utilsImport.stringVal(this[next[key]], 0);
        }, nodes);
        ...
        if (directions.length) {
            ...
            nodes[position] = utilsImport.replaceAt(nodes[position], positionIndex[direction], 0);
            // Set new position
            position = next[direction];

            // Update next position
            nodes[position] = utilsImport.replaceAt(nodes[position], oppositeIndex[direction], 0);
            nodes[position] = utilsImport.replaceAt(nodes[position], 0, 1);
        } else {
            if (!moveNodes.length) {
                break;
...
Maze.prototype.getMatrix = function (nodes) {
    ...
    for (let i = 0; i < mazeSize; i++) {
        ...
        if (utilsImport.stringVal(nodes[i], 1)) {
            row1 += '11';
            if (utilsImport.stringVal(nodes[i], 4)) {
                row2 += '01';
            } else {
                row2 += '00';
            }
            ...
            let above = hasAbove && utilsImport.stringVal(nodes[i - this.width], 4);
            let hasNext = nodes.hasOwnProperty(i + 1);
            let next = hasNext && utilsImport.stringVal(nodes[i + 1], 1);
...
Maze.prototype.removeWall = function (y, i) {
    ...
    const even = (y % 2 === 0)
    const wall = utilsImport.stringVal(this.matrix[y], i);
    ...
    if (!even && (i % 2 === 0)) {
        ...
        const hasTop = (y - 2 > 0) && (1 === utilsImport.stringVal(this.matrix[y - 2], i));
        const hasBottom = (y + 2 < this.matrix.length) && (1 === utilsImport.stringVal(this.matrix[y + 2], i));

        if (hasTop && hasBottom) {
            this.matrix[y] = utilsImport.replaceAt(this.matrix[y], i, '0');
            return true;
        } else if (!hasTop && hasBottom) {
            const left = 1 === utilsImport.stringVal(this.matrix[y - 1], i - 1);
            const right = 1 === utilsImport.stringVal(this.matrix[y - 1], i + 1);
            if (left || right) {
                this.matrix[y] = utilsImport.replaceAt(this.matrix[y], i, '0');
                return true;
            }
        ...
        const hasLeft = 1 === utilsImport.stringVal(this.matrix[y], i - 2);
        const hasRight = 1 === utilsImport.stringVal(this.matrix[y], i + 2);

        if (hasLeft && hasRight) {
            this.matrix[y] = replaceAt(this.matrix[y], i, '0');
            return true;
        } else if (!hasLeft && hasRight) {
            const top = 1 === utilsImport.stringVal(this.matrix[y - 1], i - 1);
            const bottom = 1 === utilsImport.stringVal(this.matrix[y + 1], i - 1);
            if (top || bottom) {
                this.matrix[y] = utilsImport.replaceAt(this.matrix[y], i, '0');
                return true;
            }
...
Maze.prototype.removeMazeWalls = function () {
    ...
    while (tries < maxTries) {
        ...
        for (let i = 0; i < row.length; i++) {
            ...
            const wall = utilsImport.stringVal(row, i);
            if (wall) {
                walls.push(i);
            }
        }
